...
from utils.train import Step2_ModelTrainer, plot_line
...
if __name__ == '__main__':
    ...
    for prefix in ['train', 'val']:
        ...
        for row in reader:
            with open(eval('{}_txt_path'.format(prefix)), 'a') as f:
                f.write(os.path.join(opt.img_aug_path, row[0]))
                f.write(' ' + row[-1])
                f.write('\n')

    train_dataset = Step2_Dataset(train_txt_path, train_transform)
    val_dataset = Step2_Dataset(val_txt_path, val_transform)
    ...
    scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, 8, 2)
    ...
    for epoch in range(start_epoch + 1, opt.max_epoch):
        if opt.use_mixup:
            ...
            mix_up = False
        loss_train, acc_train, f1_train = Step2_ModelTrainer().train(train_dl, model, criterion, optimizer, device, epoch,
                                                                  opt.max_epoch, num_cls, opt.print_acc_iter, mix_up,
                                                                  log_file)
        loss_val, acc_val, f1_val = Step2_ModelTrainer().valid(val_dl, model, criterion, device, epoch,
