...
if __name__ == '__main__':
    ...
    train_dataset = FGVC8_Dataset_MultiClass(train_txt_path, train_transform)
    val_dataset = FGVC8_Dataset_MultiClass(val_txt_path, val_transform)
    ...
    for epoch in range(start_epoch + 1, opt.max_epoch):
        if opt.use_mixup:
            ...
            mix_up = False
        loss_train, acc_train, f1_train = ModelTrainer().train(train_dl, model, criterion, optimizer, device, epoch,
                                                                  opt.max_epoch, num_cls, opt.print_acc_iter, mix_up,
                                                                  log_file)
        loss_val, acc_val, f1_val = ModelTrainer().valid(val_dl, model, criterion, device, epoch,
