...
if __name__ == '__main__':
    img_cls = {'complex': 0, 'frog_eye_leaf_spot': 1, 'powdery_mildew': 2, 'rust': 3, 'scab': 4}  # class:label_number
    ...
    for row in tqdm(reader):
        ...
        if len(label) > 0 and label.find(' ') == -1:
            if label == 'healthy':
                continue
            save_pth = os.path.join(img_aug_save_path, str(img_cls[label]) + label)
